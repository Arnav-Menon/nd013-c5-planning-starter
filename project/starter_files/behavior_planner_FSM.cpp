...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  auto look_ahead_distance = pow((velocity_mag, 2) / 2) * accel_mag;  // <- Fixed This
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      goal.location.x += (_stop_line_buffer * std::cos(ang));  // <- Fixed This
      goal.location.y += (_stop_line_buffer * std::sin(ang));  // <- Fixed This
    ...
    // if (utils::magnitude(ego_state.velocity) <= _stop_threshold_speed) {  // -> Fixed this
